...
def welcome():
    ...
    query,res = checkPermitCookie(key,admin_permit)
    #获取当前cookie
    response=make_response(redirect('/'))#重定向
    admin_permit_now_cookie = request.cookies.get('admin_permit')

    if admin_permit_now_cookie == None and res:
            permit = json.dumps(query)
            response.set_cookie('admin_permit',permit)
    if res :
        #种当前用户的uid cookie
        m2 = hashlib.md5()
        md5_src =current_app.config['COOKIE_PREFIX'] + 'uid' + query['uid'] + current_app.config['COOKIE_KEY']
        m2.update(md5_src.encode("utf-8"))
        pro_key = m2.hexdigest()
...
#验证登录并设置cookie
def checkPermitCookie(key,admin_permit): 
    #urldecode
    admin_permit_str = parse.unquote(admin_permit)
    #url参数生成dict
    query = dict(parse.parse_qsl(admin_permit_str))

    if key !=None and query['uid'] !=None:
        #生成md5加密
        m2 = hashlib.md5()
        md5_src = key+'-'+query['lt']+'-'+query['uid']
        m2.update(md5_src.encode("utf-8"))
        pro_key = m2.hexdigest()
         
        tmp_key = pro_key[2:1] + pro_key[7:1] + pro_key[17:1] + pro_key[25:1] + pro_key[31:1]
        key_pos=query['permit'].find(tmp_key)
        ticks = time.time()
        if key_pos != None and key_pos % 5 == 0  and ticks-int(query['lt']) < 72000:
            return query,True
...
#登录是判断用户登录状态
def checkLogin():
    value = request.cookies.get(current_app.config['COOKIE_PREFIX']+'uid')
    hash = request.cookies.get(current_app.config['COOKIE_PREFIX']+'uid'+'_hash')
    m2 = hashlib.md5()
    md5_src =current_app.config['COOKIE_PREFIX'] + 'uid' + str(value) + current_app.config['COOKIE_KEY']
    m2.update(md5_src.encode("utf-8"))
    hash_v = m2.hexdigest()
